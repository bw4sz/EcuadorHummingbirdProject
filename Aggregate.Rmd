...
select(date,Month,Year,site,plant_field_name,total_flowers,waypoint,height,hummingbird,comment)

#fix mistakes
levels(transect_data$plant_field_name)<-
transect_data$plant_field_name %in% 
```
...
cam_levels<-factor(camera_dat$plant_field_name)
...
camera_dat<-camera_dat %>% filter(plant_field_name %in% plants_keep)
```
...
```{r}
##need to standardize plant species to read from revised plant names
int_levels<-factor(int_data$hummingbird)
...
#only hummingbirds
isclass<-tax_name(query = hum_keep, get = "family", db = "ncbi")
hum_keep<-hum_keep[isclass$family %in% "Trochilidae"]

#get taxize
int_data<-int_data %>% filter(hummingbird %in% hum_keep)
```
