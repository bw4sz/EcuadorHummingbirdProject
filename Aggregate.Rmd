...
```{r,eval=T}
taxsheet<-read_excel("/Users/ben/Dropbox/Lista Planta final/LISTA PLANTA_FINAL_TODOS_150119.xls") %>% select(Site,"field_name (AyC)","ID_final") %>% rename(plant_field_name=`field_name (AyC)`,final_plant_name=`ID_final`)
...
plant_tax_needed<-transect_data %>% filter(final_plant_name %in% plants_missing,!is.na(final_plant_name),!final_plant_name %in% c(" ","")) %>% distinct(site,final_plant_name)
